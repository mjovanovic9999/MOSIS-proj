...
@Composable
fun SignInScreen(
    viewModel: SignInViewModel, destinationsNavigator: DestinationsNavigator
) {
        ...
        state.signInScreenEvents.collect {
            if (it != null) when (it) {
                ...
                SignInScreenEvents.EmailNotVerified -> {
                    destinationsNavigator.popBackStack()
                    destinationsNavigator.navigate(TikiScreenDestination(contentType = TikiScreenContentType.EmailVerification))
                }
            }
        }
    }
            ...
            CustomButton(
                clickHandler = {
                    destinationsNavigator.navigate(
                        TikiScreenDestination(
                            contentType = TikiScreenContentType.PasswordReset
                        )
                    )
                },
    ...
    OneTapGoogle(signInResult = state.onTapSignInResponse.value,
